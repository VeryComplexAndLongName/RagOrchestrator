...
GitLab RAG Demo
===============
Ingest public GitLab repositories into a local SQLite+vec store and a graph store,
...
# Ingest and ask a single question (non-interactive):
    python scripts/gitlab_demo/run.py --groups-or-users gitlab-org python --ask "What is this repository about?"

# Interactive REPL (keeps asking until you type 'exit' or Ctrl-C):
    python scripts/gitlab_demo/run.py --groups-or-users gitlab-org python --interactive

# Skip ingest if DB already exists:
    python scripts/gitlab_demo/run.py --skip-ingest --ask "What does the project do?"

Flags
-----
--groups-or-users   list of GitLab groups/users to ingest       (default: gitlab-org python)
--base-url          GitLab base URL                             (default: https://gitlab.com)
--db                path to SQLite+vec database file            (default: scripts/gitlab_demo/gitlab.sqlite)
--graph-db          path to repository graph database           (default: scripts/gitlab_demo/gitlab_graph.sqlite)
...
--token             GitLab token for higher rate limits
...
from rag_orchestrator.graph import SqlGraphStore  # noqa: E402
...
from rag_orchestrator.templates.gitlab_template import GitLabTemplate  # noqa: E402
from rag_orchestrator.templates.models import GitLabConfig  # noqa: E402
...
class OllamaChatGenerator:
    ...
    SYSTEM_PROMPT = (
        "You are an expert on GitLab repositories. "
...
def build_orchestrator(db_path: str, embed_model: str, ollama_url: str) -> RAGOrchestrator:
    ...
    provider = create_provider("sqlite+vec", db_path=db_path, table_name="gitlab_chunks")
...
def ingest_repositories(
    ...
    config = GitLabConfig(
        base_url=base_url,
        groups_or_users=groups_or_users,
    ...
    report = GitLabTemplate(orchestrator, graph_store=graph_store).run(config)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="GitLab RAG demo: ingest repositories and ask questions via Ollama",
    ...
    parser.add_argument(
        "--groups-or-users",
        nargs="+",
        default=["gitlab-org", "python"],
        metavar="GROUP_OR_USER",
        help="GitLab groups or users to ingest",
    )
    parser.add_argument(
        "--base-url",
    ...
    parser.add_argument(
        "--db",
        default=str(Path(__file__).parent / "gitlab.sqlite"),
        help="SQLite+vec DB path (default: scripts/gitlab_demo/gitlab.sqlite)",
    )
    parser.add_argument(
        "--graph-db",
        default=str(Path(__file__).parent / "gitlab_graph.sqlite"),
        help="Repository graph DB path (default: scripts/gitlab_demo/gitlab_graph.sqlite)",
    )
    parser.add_argument(
        "--embed-model",
    ...
    parser.add_argument("--token", default=os.environ.get("GITLAB_TOKEN", ""), help="GitLab token")
    parser.add_argument(
        "--include-readme",
    ...
    if not args.skip_ingest:
        print(f"Ingesting {len(args.groups_or_users)} group/user entries: {', '.join(args.groups_or_users)} ...")
        ingest_perf = ingest_repositories(
            orchestrator=orchestrator,
            graph_store=graph_store,
            groups_or_users=args.groups_or_users,
