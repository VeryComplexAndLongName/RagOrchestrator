...
======================
Ingest OpenAPI/Swagger specs (URL or local file) into a local SQLite+vec store,
...
--db                 path to SQLite+vec database file            (default: scripts/api_demo/api.sqlite)
...
def build_orchestrator(db_path: str, embed_model: str, ollama_url: str) -> RAGOrchestrator:
    ...
    provider = create_provider("sqlite+vec", db_path=db_path, table_name="api_chunks")
...
def main() -> int:
    ...
    parser.add_argument(
        "--db",
        default=str(Path(__file__).parent / "api.sqlite"),
        help="SQLite+vec DB path (default: scripts/api_demo/api.sqlite)",
    )
    parser.add_argument(
        "--embed-model",
