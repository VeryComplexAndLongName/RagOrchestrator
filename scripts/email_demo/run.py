...
Usage
-----
# Ingest and ask a single question:
    python scripts/email_demo/run.py --sources d:/TEMP/tickets --ask "Какие основные проблемы у пользователей?"

# Interactive REPL:
    python scripts/email_demo/run.py --sources d:/TEMP/tickets --interactive

# Skip ingest if DB already exists:
    python scripts/email_demo/run.py --skip-ingest --ask "Какие тикеты самые частые?"
"""
...
from ragflow_orchestrator.templates.email_ticket import EmailTicketTemplate  # noqa: E402
from ragflow_orchestrator.templates.models import EmailTicketConfig  # noqa: E402
...
class OllamaChatGenerator:
    SYSTEM_PROMPT = (
        "You are an expert on support tickets. "
...
def ingest_tickets(orchestrator: RAGOrchestrator, sources: list[str], recursive: bool, extensions: list[str]) -> IngestPerf:
    normalized_ext = [ext if ext.startswith(".") else f".{ext}" for ext in extensions]
    config = EmailTicketConfig(sources=sources, recursive=recursive, extensions=normalized_ext)

    t0 = time.perf_counter()
    report = EmailTicketTemplate(orchestrator).run(config)
    ...
    return IngestPerf(
        tickets_ingested=len(report.ingested),
...
def print_ingest_perf(perf: IngestPerf) -> None:
    ...
    _hr()
    print(f"  tickets ingested     : {perf.tickets_ingested}")
...
def print_answer(result: QueryAnswer) -> None:
    ...
    for hit in result.context:
        ticket_id = str(hit.chunk.metadata.get("ticket_id") or hit.chunk.source_id)
        if ticket_id in seen:
            continue
        seen.add(ticket_id)
        print(f"  - {ticket_id}  (score {hit.score:.4f})")
...
def main() -> int:
    ...
    parser.add_argument("--extensions", nargs="+", default=[".eml", ".jsonl", ".csv", ".txt", ".md"], metavar="EXT", help="File extensions to include")
    ...
    if not args.skip_ingest:
        ...
        ingest_perf = ingest_tickets(orchestrator, args.sources, args.recursive, args.extensions)
